...
class CreateCoursePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.preview_upload_icon = page.get_by_test_id('create-course-preview-image-upload-widget-info-icon')
        self.preview_upload_title = page.get_by_test_id('create-course-preview-image-upload-widget-info-title-text')
        self.preview_upload_description = page.get_by_test_id('create-course-preview-image-upload-widget-info-description-text')
        self.preview_upload_button = page.get_by_test_id('create-course-preview-image-upload-widget-upload-button')
        self.preview_upload_remove_button = page.get_by_test_id('create-course-preview-image-upload-widget-remove-button')
    ...
    def check_visible_image_upload_view(self, is_image_uploaded: bool = False):
        expect(self.preview_upload_icon).to_be_visible()

        expect(self.preview_upload_title).to_be_visible()
        expect(self.preview_upload_title).to_have_text('Tap on "Upload image" button to select file')

        expect(self.preview_upload_description).to_be_visible()
        expect(self.preview_upload_description).to_have_text('Recommended file size 540X300')

        expect(self.preview_upload_button).to_be_visible()

        if is_image_uploaded:
            expect(self.preview_upload_remove_button).to_be_visible()

    def click_remove_image_button(self):
        self.preview_upload_remove_button.click()
    ...
    def check_visible_create_course_form(
        ...
        expect(self.create_course_title).to_be_visible()
        expect(self.create_course_title).to_have_text(title)
